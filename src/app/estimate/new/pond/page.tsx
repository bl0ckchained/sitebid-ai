...
export default function NewPondEstimate() {
  ...
  return (
    <main className="mx-auto max-w-6xl p-6">
      ...
      <p className="text-slate-600 mt-1">Bowl cut (frustum), on-site shaping, optional beach, stone ring, overflow pipe, clay import, and permit fees.</p>
        ...
        <section className="space-y-4">
          <Card title="Geometry & Slopes">
            ...
            <Row label="Side slope (H:1)">
              <input type="number" step="0.1" className="input" value={v.side_slope_H_per_V} onChange={e=>setV({...v,side_slope_H_per_V:num(e)})}/>
            </Row>
          </Card>
          ...
          <Card title="Overflow Pipe & Appurtenances">
            <Row label="Pipe length (ft)">
              <input type="number" className="input" value={x.overflow_pipe_len_ft ?? 0} onChange={e=>setX({...x,overflow_pipe_len_ft:num(e)})}/>
            </Row>
            <Row label="Pipe Ø (in)">
              <input type="number" className="input" value={x.overflow_pipe_d_in ?? 0} onChange={e=>setX({...x,overflow_pipe_d_in:num(e)})}/>
            </Row>
            <Row label="End sections (ea)">
              <input type="number" className="input" value={x.end_sections_ea ?? 0} onChange={e=>setX({...x,end_sections_ea:num(e)})}/>
            </Row>
            <Row label="Pipe ($/ft)">
          ...
          <Card title="Beach (Sand)">
            ...
            <Row label="Width (ft)">
              <input type="number" className="input" value={x.beach_width_ft ?? 0} onChange={e=>setX({...x,beach_width_ft:num(e)})}/>
            </Row>
            <Row label="Thickness (ft)">
              <input type="number" step="0.1" className="input" value={x.beach_thk_ft ?? 0} onChange={e=>setX({...x,beach_thk_ft:num(e)})}/>
            </Row>
            <Row label="Sand ($/yd³)">
          ...
          <Card title="Decorative Stone Ring">
            <Row label="Length (ft)">
              <input type="number" className="input" value={x.decorative_len_ft ?? 0} onChange={e=>setX({...x,decorative_len_ft:num(e)})}/>
            </Row>
            <Row label="Width (ft)">
              <input type="number" className="input" value={x.decorative_width_ft ?? 0} onChange={e=>setX({...x,decorative_width_ft:num(e)})}/>
            </Row>
            <Row label="Thickness (ft)">
              <input type="number" step="0.1" className="input" value={x.decorative_thk_ft ?? 0} onChange={e=>setX({...x,decorative_thk_ft:num(e)})}/>
            </Row>
            <Row label="Stone ($/ton)">
              <input type="number" className="input" value={r.stone_ton} onChange={e=>setR({...r,stone_ton:num(e)})}/>
            </Row>
            <Row label="Density (t/yd³)">
              <input type="number" step="0.01" className="input" value={r.stone_density_ton_per_yd3} onChange={e=>setR({...r,stone_density_ton_per_yd3:num(e)})}/>
            </Row>
          </Card>

          <Card title="Clay Import (rare)">
            <Row label="Qty (yd³)">
              <input type="number" className="input" value={x.clay_import_yd3 ?? 0} onChange={e=>setX({...x,clay_import_yd3:num(e)})}/>
            </Row>
            <Row label="Clay ($/yd³)">
        ...
        <section className="space-y-4">
          ...
          <Card title="Assumptions (auto-print on quote)">
            <ul className="list-disc ml-5 text-sm">
              <li>Frustum bowl from top {v.L_top_ft}×{v.W_top_ft} ft to bottom ~{calc.Lb.toFixed(0)}×{calc.Wb.toFixed(0)} ft @ depth {v.depth_ft} ft, slopes {v.side_slope_H_per_V}:1.</li>
              <li>Haul-off fraction {Math.round(v.haul_off_fraction*100)}%, truck {v.truck_capacity_yd3} yd³, round-trip {v.haul_round_trip_min} min.</li>
              <li>Optional pipe {x.overflow_pipe_len_ft ?? 0} ft @ Ø {x.overflow_pipe_d_in ?? 0}" with {x.end_sections_ea ?? 0} end sections.</li>
              <li>Beach {x.beach_len_ft ?? 0}×{x.beach_width_ft ?? 0} ft × {(x.beach_thk_ft ?? 0)} ft; decorative stone {x.decorative_len_ft ?? 0}×{x.decorative_width_ft ?? 0} ft × {(x.decorative_thk_ft ?? 0)} ft.</li>
          ...
          <Card title="Inclusions / Exclusions (draft)">
            ...
            <p className="text-sm mt-1">
              <b>Exclusions:</b> rock excavation, dewatering, utility relocations, erosion control, landscaping, fencing, water fill, permits/fees (listed separately), items not specifically described.
            </p>
          </Card>
        </section>
      </div>
